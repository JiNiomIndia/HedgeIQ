...
const PositionDrawer = ({ sym, onClose, nav }) => {
  if (!sym) return null;
  const t = TICKERS[sym];
    ...
    <div style={{ gridColumn: '1 / -1', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: 0, marginTop: 6 }}>
      <div className="flex items-center" style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)' }}>
        ...
        <div className="flex-col" style={{ marginRight: 16 }}>
          <div style={{ fontWeight: 700, fontSize: 'var(--fs-lg)' }}>{sym} <span style={{ color: 'var(--text-subtle)', fontWeight: 500 }}>· {t.name}</span></div>
          <div className="flex gap-3" style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>
            <span>{t.sector}</span>
            <span>Vol {fmtCompact(t.vol)}</span>
            <span>Mkt cap {fmtCompact(t.mktCap)}</span>
          </div>
        </div>
        <div className="flex-col">
          <div className="mono" style={{ fontSize: 'var(--fs-xl)', fontWeight: 700 }}>{fmtMoney(t.price)}</div>
          <div className="mono" style={{ fontSize: 'var(--fs-xs)', color: t.chg >= 0 ? 'var(--pos)' : 'var(--neg)' }}>{t.chg >= 0 ? '+' : ''}{fmtMoney(t.chg)} ({fmtPct(t.chgPct)})</div>
        </div>
        <div className="grow"/>
        <div className="flex gap-2">
          <button className="btn btn-pos btn-sm" onClick={() => nav('trade', { symbol: sym, side: 'buy' })}>Buy</button>
          <button className="btn btn-neg btn-sm" onClick={() => nav('trade', { symbol: sym, side: 'sell' })}>Sell</button>
          <button className="btn btn-sm">Set exit plan</button>
          <button className="btn btn-sm" onClick={() => nav('options', { symbol: sym })}>Option chain</button>
          <button className="btn btn-ghost btn-sm" onClick={onClose}><I.X className="icon-sm"/></button>
        </div>
      </div>
      <div className="tabs" style={{ padding: '0 16px' }}>
        {['chart','research','news','fundamentals','purchase'].map(k => (
          <div key={k} className={cls('tab', tab === k && 'active')} onClick={() => setTab(k)}>
      ...
      {tab === 'chart' && (
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 0 }}>
          <div style={{ padding: 16, borderRight: '1px solid var(--border)' }}>
            <CandleChart candles={candles} w={520} h={240}/>
          </div>
          <div style={{ padding: 16, borderRight: '1px solid var(--border)' }}>
            <div style={{ fontSize: 10, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>Quote</div>
            {[
              ['Bid', fmtMoney(t.price - 0.04) + ' × 4,800'],
              ['Ask', fmtMoney(t.price + 0.06) + ' × 100'],
              ['Volume', fmtCompact(t.vol)],
              ['90 day avg', fmtCompact(t.vol * 0.9)],
              ['Open', fmtMoney(t.price - t.chg)],
              ['Prev close', fmtMoney(t.price - t.chg)],
              ['Day range', fmtMoney(t.price * 0.99) + ' – ' + fmtMoney(t.price * 1.01)],
              ['52w range', fmtMoney(t.low52) + ' – ' + fmtMoney(t.high52)],
          ...
          <div style={{ padding: 16 }}>
            <div style={{ fontSize: 10, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>News</div>
            {NEWS.filter(n => n.sym === sym || !n.sym).slice(0, 4).map((n, i) => (
              <div key={i} style={{ padding: '8px 0', borderBottom: i < 3 ? '1px solid var(--border)' : 0 }}>
                <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 500, lineHeight: 1.4 }}>{n.t}</div>
      ...
      {tab === 'research' && (
        <div style={{ padding: 24, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20 }}>
          <div>
            <div style={{ fontSize: 10, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>Analyst ratings</div>
            <div className="mono" style={{ fontSize: 28, fontWeight: 700, color: 'var(--pos)' }}>1.3</div>
            <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>Strong Buy · 42 analysts</div>
            <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 4 }}>
              {[['Buy', 28, 'var(--pos)'], ['Hold', 10, 'var(--text-muted)'], ['Sell', 4, 'var(--neg)']].map(([k, v, c]) => (
                <div key={k} className="flex items-center gap-2" style={{ fontSize: 'var(--fs-xs)' }}>
                  <span style={{ width: 36 }}>{k}</span>
                  <div style={{ flexGrow: 1, height: 6, background: 'var(--surface-sunken)', borderRadius: 3 }}>
                    <div style={{ width: `${v * 2.3}%`, height: '100%', background: c, borderRadius: 3 }}/>
                  </div>
                  <span className="mono" style={{ width: 20, textAlign: 'right' }}>{v}</span>
                </div>
              ))}
            </div>
          </div>
          <div>
            <div style={{ fontSize: 10, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>Price target</div>
            <div className="mono" style={{ fontSize: 22, fontWeight: 700 }}>{fmtMoney(t.price * 1.18)}</div>
            <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--pos)' }}>+{((1.18 - 1) * 100).toFixed(1)}% upside</div>
            <div style={{ marginTop: 14 }}>
              <div className="flex justify-between" style={{ fontSize: 10, color: 'var(--text-subtle)', marginBottom: 4 }}>
                <span>Low {fmtMoney(t.price * 0.88)}</span>
                <span>High {fmtMoney(t.price * 1.32)}</span>
              </div>
              <RangeBar low={t.price * 0.88} high={t.price * 1.32} current={t.price * 1.18} w={220} compact/>
            </div>
          </div>
          <div>
            <div style={{ fontSize: 10, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>AI summary</div>
            <div style={{ fontSize: 'var(--fs-xs)', lineHeight: 1.6, color: 'var(--text-muted)' }}>
              {sym} is trading near its all-time high on strong AI demand. Earnings beat last quarter by 12%. Options implied vol is elevated at 38%, above 30-day avg of 32%. Sector rotation risk if rates rise.
            </div>
          </div>
        </div>
      )}
      {(tab === 'news' || tab === 'fundamentals' || tab === 'purchase') && (
        <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)', fontSize: 'var(--fs-sm)' }}>
          {tab[0].toUpperCase() + tab.slice(1)} panel (demo placeholder)
        </div>
      )}
    </div>
  );
                            ...
                            {isExp && (
                              <tr className="expanded">
                                <td colSpan={13} style={{ padding: 0 }}>
                                  <PositionDrawer sym={p.sym} onClose={() => setExpanded(null)} nav={nav}/>
                                </td>
                              </tr>
                            )}
                          </React.Fragment>
