...
DESIGN-FIRST PROTOCOL (critical):
  ...
  CRITICAL — which variant is authoritative:

    Three design variants exist at repo root in Claude Designs/:
      - HedgeIQ_Basic/          Classic v1 (oldest, simplest)
      - HedgeIQ_Adnl_Features/  Classic + OptionChain + Research
      - HedgeIQ_Futuristic/     Latest — superset of both Classics
                                PLUS adds AI-native variants (purple
                                accent) for Trade and Options flows

    The Futuristic variant is extracted into docs/design/ and is the
    AUTHORITATIVE visual spec for Phase 2.

    Futuristic is a strict superset of Basic:
      - Every Classic file (dashboard.jsx, shell.jsx, charts.jsx,
        screens.jsx, data.jsx) is unchanged from Basic and ported as-is.
      - app.jsx extended (362 → 406 lines): theme switcher supports
        meridian/lumen/terminal; adds futuristic-mode toggle.
      - positions.jsx extended (321 → 413 lines): adds density modes,
        colorblind palette, inline sparkline enhancements.
      - trade.jsx MAJOR expansion (302 → 896 lines): classic form
        PLUS the AI-native trade experience co-exist.
      - util.jsx +1 line (minor helper).
      - NEW file futuristic.jsx (955 lines): AI-native Buy/Sell UI.
      - NEW file futuristic-mode.jsx (1033 lines): AI-native Options
        chain UI with natural-language filters.

    So: porting docs/design/src/*.jsx VERBATIM captures every Classic
    feature + every Futuristic addition simultaneously. Do not attempt
    to merge Basic and Futuristic — Futuristic already contains both.

  Classic-only flows (not yet given a Futuristic redesign):

    User confirmation: "not all the flows and screens from Classic are
    converted to Futuristic designs by Claude Design yet." The user
    will add more Futuristic designs later. For now:

    - EVERY feature present in Classic (Basic + Adnl_Features) must be
      implemented in HedgeIQ, styled using the 3-theme CSS variable
      system from theme.css (meridian/lumen/terminal).
    - Where Futuristic offers a redesigned variant of a flow (Trade,
      Options), implement BOTH the Classic UI and the Futuristic UI.
      Expose them via the futuristic-mode toggle already present in
      app.jsx — user picks which experience they want.
    - When the user later drops new Futuristic variants into
      docs/design/, running Phase 2 again replaces the corresponding
      Classic components.

  Steps:

    a. READ docs/design/COMPONENT_MAP.md first — this file already exists
       and enumerates every feature in the Futuristic prototype with the
       session number it maps to. 18 sessions total (was 10 — expanded
       after a detailed audit of the prototype revealed 40+ features the
       original plan under-specified). Treat COMPONENT_MAP.md as the
       authoritative feature checklist. Every row marked ❌ must be
       implemented; every row marked ⚠️ must have the extensions called
       out added; ✅ rows can be verified only.
    b. Session 1: copy theme.css → frontend/src/styles/theme.css.
       Parse tokens.json → generate frontend/src/lib/theme.ts with
       a type-safe token object. Set up ThemeProvider to toggle
       [data-theme="meridian" | "lumen" | "terminal"] on <html>.
       Use the designer's actual theme names — don't rename them.
       Default theme = meridian (matches <body data-theme="meridian">
       in docs/design/HedgeIQ.html).
       Also implement the density toggle (balanced/dense/airy) and
       futuristic-mode toggle from app.jsx's TweaksPanel.
    c. Each subsequent session: for each component it builds, first
       locate the corresponding JSX block in docs/design/src/*.jsx,
    ...
    d. For flows with Futuristic variants (Trade, Options): build both
       the Classic version and the Futuristic version. Route based on
       the user's futuristic-mode preference.
    e. When the prototype lacks a component HedgeIQ needs (unlikely
       given Futuristic's coverage, but possible), fall back to PNG
       references and Fidelity screenshots per the standard flow.
    f. Preserve the designer's AI Copilot right-rail (app.jsx lines
       ~185-275) as the HedgeIQ AI Advisor widget — the prototype
       already implements streaming bubbles, suggestions, inline
       insights cards exactly matching user's earlier feedback.
