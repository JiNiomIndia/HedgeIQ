...
const Sidebar = ({ nav, current, onOpenCopilot }) => {
  const items = [
    ...
    { k: 'trade', lbl: 'Trade', I: I.Bolt, isAction: true },
    ...
    <nav style={{ width: 66, flexShrink: 0, background: 'var(--surface)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px 0', gap: 2 }}>
      ...
      {items.map(it => (
        <button key={it.k} onClick={() => {
          if (it.isAction) window.openTrade?.();
          else if (it.k === 'research') window.openResearch?.();
          else if (it.k === 'options') window.openOptionChain?.();
          else nav(it.k);
        }} title={it.lbl}
...
const TopBar = ({ nav, onOpenCopilot, tweaks, setTweaks, mode, setMode }) => {
  ...
  return (
    <header style={{ height: 52, borderBottom: '1px solid var(--border)', background: 'var(--surface)', display: 'flex', alignItems: 'center', padding: '0 16px', gap: 16, flexShrink: 0, position: 'relative', zIndex: 20 }}>
      ...
      {/* Mode toggle */}
      <div style={{ display:'inline-flex', padding:3, background:'var(--surface-sunken)', border:'1px solid var(--border)', borderRadius:8, gap:2 }}>
        <button onClick={() => setMode('classic')}
          style={{ padding:'3px 10px', fontSize:10.5, fontWeight:700, letterSpacing:.4, borderRadius:5,
                   background: mode==='classic' ? 'var(--accent)' : 'transparent',
                   color: mode==='classic' ? 'var(--accent-contrast)' : 'var(--text-muted)',
                   border:'none', cursor:'pointer' }}>CLASSIC</button>
        <button onClick={() => setMode('futuristic')}
          style={{ padding:'3px 10px', fontSize:10.5, fontWeight:700, letterSpacing:.4, borderRadius:5,
                   background: mode==='futuristic' ? 'linear-gradient(135deg, #6B4FFF, #8F7BFF)' : 'transparent',
                   color: mode==='futuristic' ? '#fff' : 'var(--text-muted)',
                   border:'none', cursor:'pointer', display:'inline-flex', alignItems:'center', gap:4 }}>
          <svg width="9" height="9" viewBox="0 0 12 12"><path d="M6 0.5L7.3 4.7L11.5 6L7.3 7.3L6 11.5L4.7 7.3L0.5 6L4.7 4.7L6 0.5Z" fill="currentColor"/></svg>
          FUTURISTIC
        </button>
      </div>
...
const App = () => {
  const [mode, setModeState] = React.useState(() => localStorage.getItem('hedgeiq.mode') || 'classic');
  const setMode = (m) => { setModeState(m); localStorage.setItem('hedgeiq.mode', m); };
  ...
  const [tradeDrawer, setTradeDrawer] = React.useState({ open: false, initial: null });

  // Expose global openTrade so any component — or even an onClick in raw HTML — can trigger the drawer
  React.useEffect(() => {
    window.openTrade = (symbol, side = 'buy') => setTradeDrawer({ open: true, initial: { symbol, side } });
    window.openResearch = (symbol) => window.open(`Research.html?symbol=${encodeURIComponent(symbol || 'NVDA')}`, '_blank', 'width=1280,height=900');
    window.openOptionChain = (symbol) => window.open(`OptionChain.html?symbol=${encodeURIComponent(symbol || 'NVDA')}`, '_blank', 'width=1400,height=900');
  }, []);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }} data-screen-label={mode==='futuristic'?`futuristic-${screen}`:screen}>
      {mode === 'futuristic' ? (
        <FuturisticMode onBackToClassic={() => setMode('classic')}/>
      ) : (
        <React.Fragment>
          <TopBar nav={nav} onOpenCopilot={() => setCopilotOpen(true)} tweaks={tweaks} setTweaks={setTweaks} mode={mode} setMode={setMode}/>
          ...
          <TradeDrawer
            ...
            initial={tradeDrawer.initial}
            onClose={() => setTradeDrawer({ open: false, initial: null })}
          />
        </React.Fragment>
      )}
    </div>
